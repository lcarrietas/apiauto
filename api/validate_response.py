...
validate_response.py
    validate todoist request response
"""
import json
import logging
...
class ValidateResponse(metaclass=Singleton):
    ...
    def validate_response(self, actual_response, method=None, expected_status_code=200,
                          feature=None):
        """
        Method to validate request response 
        """
        ...
        if 'body' in actual_response and actual_response["body"] is not None:
            ...
            self.validate_value(actual_response["status"],
                                input_data["response"]["status"], field="status")
            self.validate_value(actual_response["headers"],
                                input_data["response"]["headers"], field="headers")
            self.validate_value(actual_response["body"],
                                input_data["response"]["body"], field="body")

    def validate_value(self, actual_value, expected_value, field):
        """
        Method to validate request body values 
        """
